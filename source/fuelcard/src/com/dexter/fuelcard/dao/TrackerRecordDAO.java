...
public class TrackerRecordDAO
{
	...
	public Vector<TrackerRecord> search(Date d1, Date d2, Car car)
    {
    ...
    @SuppressWarnings("unchecked")
	public Vector<TrackerRecord> search(Date d1, Date d2, Long partner_id)
    {
    	Vector<TrackerRecord> list = new Vector<TrackerRecord>();
    	...
    	try
    	{
    		...
    		if(partner_id != null)
    		{
	    		q = em.createQuery("Select e from TrackerRecord e where e.vehicle.partner.id=:partner_id and (e.tranTime between :stdt and :eddt) order by e.tranTime");
    		...
    		{
    			q = em.createQuery("Select e from TrackerRecord e where (e.tranTime between :stdt and :eddt) order by e.tranTime");
        		q.setParameter("stdt", d1);
        		q.setParameter("eddt", d2);
    		}
