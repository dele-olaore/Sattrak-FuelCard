...
public class ReportDAO
{
		...
		try
		{
			...
			if(brList.size() == trList.size())
			{
				for(int i=0; i<brList.size(); i++)
				{
					...
					e[12] = car.getRegNumber();
					e[14] = (car.getRegion()!=null)?car.getRegion().getName():"N/A";
					if(curBankRec.getVehicle() != null && curBankRec.getVehicle().getAssignedUser() != null)
						e[13] = curBankRec.getVehicle().getAssignedUser().getFullname();
					else
						e[13] = "N/A";
			...
			else
			{
				for(int i=0; i<trList.size(); i++)
				{
					...
					e[14] = (car.getRegion()!=null)?car.getRegion().getName():"N/A";
					e[12] = car.getRegNumber();
					if(curTrackerRec.getVehicle() != null && curTrackerRec.getVehicle().getAssignedUser() != null)
						e[13] = curTrackerRec.getVehicle().getAssignedUser().getFullname();
					else
						e[13] = "N/A";
	...
	@SuppressWarnings("deprecation")
	public ArrayList<String[]> exceptionTransactions(Date d1, Date d2, Car car)
	{
		ArrayList<String[]> list = new ArrayList<String[]>();
			...
			for(TrackerRecord tr : trList)
			{
				if(tr.getVehicle() != null)
				{
                    ...
                    if(fl.doubleValue() > tr.getVehicle().getCalibratedCapacity())
					{
						...
						e[1] = tr.getVehicle().getRegNumber();
						e[2] = tr.getTranTime().toLocaleString();
						e[3] = "Final fuel level: " + NumberFormat.getInstance().format(fl.doubleValue()) + ", Calibrated capacity: " + tr.getVehicle().getCalibratedCapacity();
						list.add(e);
					}
				}
			}
	...
	public ArrayList<String[]> highestFuelConsumption(Date d1, Date d2)
	{
		...
		try
		{
			...
			for(int i=(comsupvalues.length-1); i>=0; i--) // take from the highest down
			{
				...
				String[] e = new String[]{vtrsList.get(0).getVehicle().getRegNumber(), ""+comsupvalues[i], (vtrsList.get(0).getVehicle().getRegion()!=null) ? vtrsList.get(0).getVehicle().getRegion().getName() : "N/A"};
				list.add(e);
